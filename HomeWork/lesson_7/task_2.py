...
class PersonInfo:
    ...
    def new_salary(self):
        ...
        for div in self.divisions:
            for letter in div:
                if letter not in our_dict:
                    our_dict[letter] = 1
                else:
                    our_dict[letter] += 1
        values = list(our_dict.values())
        sum_letter = sum(sorted(values, reverse=True)[:3])
        salary = 1337 * sum_letter * self.age
        return salary
