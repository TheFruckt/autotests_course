...
class Segment:
    ...
    def length(self):
        segm = round(math.sqrt((self.x2 - self.x1)**2 + (self.y2 - self.y1)**2), 2)
        return segm

    def x_axis_intersection(self):
        if self.y1 == 0 or self.y2 == 0 or (self.y1*self.y2 < 0):
            return True
        else:
            return False

    def y_axis_intersection(self):
        if self.x1 == 0 or self.x2 == 0 or (self.x1*self.x2 < 0):
            return True
        else:
            return False
